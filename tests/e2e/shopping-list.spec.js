...
test.describe('Shopping List Feature', () => {
  ...
  test('user can add meal to shopping list from meal detail page', async ({ page }) => {
    ...
    await expect(successToast).toContainText('Added to shopping list', { timeout: 2000 });
  });
  ...
  test('user can view shopping list items', async ({ page }) => {
    ...
    // Should have Shopping List heading
    await expect(page.locator('h1:has-text("Shopping List")')).toBeVisible();
    
    // Should have list items container
  ...
  test('user can toggle item as purchased', async ({ page }) => {
    ...
    // Add a meal to shopping list first
    await page.goto('/meal.php?id=1');
    await page.waitForLoadState('networkidle');
    const addBtn = page.locator('button:has-text("Shopping List")').first();
  ...
  test('user can add custom item to shopping list', async ({ page }) => {
    ...
    const addBtn = page.locator('button:has-text("Add"):has-text("Add Item")').last();
  ...
  test('shopping list items grouped by category', async ({ page }) => {
    ...
    for (const id of mealIds) {
      await page.goto(`/meal.php?id=${id}`);
      const addBtn = page.locator('button:has-text("Shopping List")').first();
      if (await addBtn.isVisible()) {
        await addBtn.click();
