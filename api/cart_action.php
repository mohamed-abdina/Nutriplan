<?php
// Cart action API (renamed from shopping_action)
...
if (!$limiter->check_rate_limit('cart_action', 10, 60)) {
    ...
    $error_logger->log_security_event('RATE_LIMIT_EXCEEDED', ['endpoint' => 'cart_action']);
    exit;
}
...
// Log API call
$error_logger->log_api_call('cart_action', 'POST', ['action' => $action], 200);
...
if ($row && isset($row['list_id'])) {
    ...
    pdo_query("INSERT INTO carts (user_id, list_name) VALUES (:user_id, :name)", [':user_id' => (int)$user_id, ':name' => 'My Cart']);
    global $pdo;
...
if ($action === 'add_to_cart') {
    ...
    if ($meal_id > 0) {
        ...
        if (!empty($ingredients)) {
            ...
            echo json_encode(['success' => true, 'message' => "Added {$meal['meal_name']} and " . count($ingredients) . " items to cart"]);
        } else {
...
elseif ($action === 'toggle_cart_item') {
...
elseif ($action === 'add') {
    // Redirect to new action name
    $_POST['action'] = 'add_to_cart';
    header('X-Legacy-API-Call: true');
    // Re-execute with new action
    $action = 'add_to_cart';
    ...
    if ($meal_id > 0) {
        ...
        $ingredients = pdo_fetch_all("SELECT ingredient_name, quantity, unit FROM ingredients WHERE meal_id = ? ORDER BY ingredient_id", [$meal_id]);
        if (!empty($ingredients)) {
            $stmtAddItem = $pdo->prepare("INSERT INTO cart_items (list_id, meal_id, item_name, quantity, custom_item) VALUES (?, ?, ?, ?, 0)");
            foreach ($ingredients as $ing) {
                $quantity_display = $ing['quantity'] . ' ' . $ing['unit'];
...
elseif ($action === 'check_in_cart') {
    // Check if a meal is already in the user's cart
    $meal_id = isset($_POST['meal_id']) ? (int)$_POST['meal_id'] : 0;
    if ($meal_id > 0) {
        $item = pdo_fetch_one("
            SELECT ci.item_id FROM cart_items ci
            JOIN carts c ON ci.list_id = c.list_id
            WHERE c.user_id = :user_id AND ci.meal_id = :meal_id
            LIMIT 1
        ", [':user_id' => (int)$user_id, ':meal_id' => $meal_id]);
        
        if ($item) {
            echo json_encode(['success' => true, 'in_cart' => true, 'item_id' => $item['item_id']]);
        } else {
