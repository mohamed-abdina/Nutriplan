...
if ($action === 'weekly_stats') {
    ...
    for ($i = 6; $i >= 0; $i--) {
        ...
        $data = pdo_fetch_one(
            ...
            FROM carts c
            LEFT JOIN cart_items si ON c.list_id = si.list_id
            LEFT JOIN meals m ON si.meal_id = m.meal_id
            LEFT JOIN nutrition n ON m.meal_id = n.meal_id
            WHERE c.user_id = :user_id AND DATE(si.created_at) = :date",
...
elseif ($action === 'meal_frequency') {
    ...
    $meals = pdo_fetch_all(
        ...
        JOIN carts c ON si.list_id = c.list_id
        LEFT JOIN meal_ratings mr ON m.meal_id = mr.meal_id AND mr.user_id = :user_id
        WHERE c.user_id = :user_id AND si.meal_id IS NOT NULL
        GROUP BY m.meal_id
...
elseif ($action === 'nutrition_trends') {
    ...
    $trend_data = pdo_fetch_one(
        ...
        LEFT JOIN meals m ON si.meal_id = m.meal_id
        LEFT JOIN nutrition n ON m.meal_id = n.meal_id
        WHERE c.user_id = :user_id AND si.created_at >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)",
        [':user_id' => $user_id]
...
elseif ($action === 'export_data') {
    ...
    $meals = pdo_fetch_all(
        ...
        JOIN carts c2 ON si.list_id = c2.list_id
