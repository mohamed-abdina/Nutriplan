...
$meal_id = (int)($_GET['meal_id'] ?? $_GET['id'] ?? 0);
...
<body>
            ...
            <div class="two-column-layout">
                ...
                <div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: var(--sp-4);" class="meal-actions">
                        ...
                        <button class="btn btn-secondary btn-full" onclick="toggleWishlist(<?php echo $meal['meal_id']; ?>)" id="favorite-btn" data-wishlist-btn="<?php echo $meal['meal_id']; ?>" aria-label="<?php echo ($user_rating && $user_rating['is_wishlisted']) ? 'Remove from wishlist' : 'Add to wishlist'; ?>" title="<?php echo ($user_rating && $user_rating['is_wishlisted']) ? 'Remove from wishlist' : 'Add to wishlist'; ?>" style="border: 2px solid var(--<?php echo $user_rating && $user_rating['is_wishlisted'] ? 'warning' : 'border'; ?>);">
            ...
            <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 14px; padding: var(--sp-6);">
                ...
                <div id="ingredients-panel" role="tabpanel" aria-labelledby="ingredients-tab" class="tab-panel active">
                    ...
                    <div style="display: flex; align-items: center; gap: var(--sp-3); margin-bottom: var(--sp-4); flex-wrap: wrap;">
                        <label for="portion-scale" style="color: var(--text-2);">Portion Multiplier</label>
                        <input id="portion-scale" type="range" min="0.5" max="3" step="0.25" value="1" oninput="updateIngredientScaling(this.value)">
                        <strong id="portion-scale-value">x1.00</strong>
                    </div>
                    ...
                    <ul style="list-style: none;">
                        ...
                        <li style="padding: var(--sp-3) 0; border-bottom: 1px solid var(--border);">
                            ...
                            <?php $base_qty = is_numeric((string)$ing['quantity']) ? (float)$ing['quantity'] : 0; ?>
                            <span class="ingredient-qty" data-base-qty="<?php echo htmlspecialchars((string)$base_qty, ENT_QUOTES, 'UTF-8'); ?>" data-raw-qty="<?php echo htmlspecialchars((string)$ing['quantity'], ENT_QUOTES, 'UTF-8'); ?>" data-unit="<?php echo htmlspecialchars((string)$ing['unit'], ENT_QUOTES, 'UTF-8'); ?>" style="float: right; color: var(--text-2);"><?php echo htmlspecialchars($ing['quantity'] . ' ' . $ing['unit']); ?></span>
                        </li>
                        <?php endforeach; ?>
                    </ul>
                    <?php else: ?>
    ...
    <script>
        ...
        function selectRating(rating) {
            ...
            function updateIngredientScaling(multiplierValue) {
                const multiplier = Number(multiplierValue) || 1;
                const label = document.getElementById('portion-scale-value');
                if (label) {
                    label.textContent = `x${multiplier.toFixed(2)}`;
                }

                document.querySelectorAll('.ingredient-qty').forEach((node) => {
                    const base = Number(node.dataset.baseQty || 0);
                    const rawQty = String(node.dataset.rawQty || '').trim();
                    const unit = String(node.dataset.unit || '').trim();

                    if (base > 0) {
                        const scaled = Math.round(base * multiplier * 100) / 100;
                        node.textContent = `${scaled} ${unit}`.trim();
                    } else {
                        node.textContent = `${rawQty} ${unit}`.trim();
                    }
                });
            }
